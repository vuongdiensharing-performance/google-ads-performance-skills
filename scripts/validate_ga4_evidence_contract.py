...
def validate_semantics(contract: dict[str, Any], provider: dict[str, Any]) -> list[str]:
    ...
    if tool not in set(provider.get("allowed_tools", [])):
        errors.append(f"source.tool is not allowed by provider policy: {tool}")
    mapped_tools = set(provider.get("evidence_types", {}).get(evidence_type, {}).get("tools", []))
    ...
    if provider.get("provenance_requirements", {}).get("request_fingerprint_required") is True:
        if provenance["request_fingerprint"] != canonical_request_fingerprint(contract):
            errors.append("provenance.request_fingerprint does not match canonical source.tool + request SHA-256")

    if provider.get("trust_boundary", {}).get("inference_used_must_be") is False and provenance["inference_used"] is not False:
...
def validate(contract: dict[str, Any], schema: dict[str, Any], provider: dict[str, Any]) -> list[str]:
    ...
    schema_errors = [error.message for error in sorted(validator.iter_errors(contract), key=lambda item: list(item.path))]
    return schema_errors or validate_semantics(contract, provider)


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the GA4 Evidence Contract.")
    parser.add_argument("--fixture", type=Path, default=ROOT / "evals/ga4/ga4_evidence_contract.yaml")
